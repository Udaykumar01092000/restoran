...
import { fetchRestaurantSpec } from '../apis/apis'; // Import the function

function Restaurantspec() {
    const dispatch = useDispatch();
    const { restaurantId, restaurantName } = useParams();
    const [restaurantSpec, setRestaurantSpec] = useState([]);
    ...
    useEffect(() => {
        ...
        fetchRestaurantSpec(restaurantId)
            .then((res) => {
                setRestaurantSpec(res.data?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards.slice(1) || []);
            })
            .catch((error) => {
